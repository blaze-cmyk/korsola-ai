...
function FormatCard({ id, label, src }: { id: string; label: string; src: string }) {
  ...
  useEffect(() => {
    ...
    return () => { if (expandTimer.current) window.clearTimeout(expandTimer.current); };
  }, [hovered]);

  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="relative aspect-[2/3]">
      <div className={`absolute inset-0 rounded-2xl overflow-hidden bg-ms-surface-2 ring-1 ring-white/5 transition-all duration-500 ease-out will-change-transform ${expanded ? 'scale-[1.08] z-30 ring-white/20 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8)]' : 'scale-100 z-0 hover:ring-white/10'}`}>
        <video ref={ref} src={src} muted={muted} loop playsInline preload="metadata" className="w-full h-full object-cover" />
        ...
        <div className="absolute top-3 left-0 right-0 px-3 flex items-center justify-center">
          <span className="text-[13px] font-semibold text-white drop-shadow-[0_2px_6px_rgba(0,0,0,0.8)] whitespace-nowrap">{label}</span>
        </div>
        <button
          onClick={(e) => { e.stopPropagation(); setMuted((m) => !m); }}
          className={`absolute top-2.5 right-2.5 grid place-items-center w-8 h-8 rounded-full bg-black/55 backdrop-blur-md text-white transition-all duration-300 ${hovered ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1 pointer-events-none'}`}
        ...
        <div className="absolute inset-x-0 bottom-0 p-2.5 overflow-hidden">
          <button
            ...
            className={`w-full h-11 rounded-full bg-white text-black text-sm font-semibold shadow-[0_10px_30px_-10px_rgba(0,0,0,0.6)] transition-all duration-300 ease-out ${hovered ? 'translate-y-0 opacity-100' : 'translate-y-[140%] opacity-0'}`}
          >
            Recreate
          </button>
        </div>
...
export function FormatsGrid() {
  return (
    <section className="px-4 md:px-8 pb-4 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {FORMATS.map((f) => <FormatCard key={f.id} id={f.id} label={f.label} src={f.src} />)}
      </div>
    </section>
  );
}
